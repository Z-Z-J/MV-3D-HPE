...
class VideoMultiViewModel(nn.Module):
    def __init__(self, cfg):
        ...
        # Spatial
        self.spatial_model = SpatialTransformer(num_joints=cfg.joints, in_chans=cfg.in_chans, embed_dim_ratio=cfg.embed_chans, 
        ...
        self.fuse_model = FuseView(in_channels=fuse_chans*2, out_channels=fuse_chans, dropout = cfg.dropout, num_joints=cfg.joints)
        
     
        # Temporal
        self.temporal_model = TemporalTransformer(num_frames=cfg.frames, embed_dim_ratio=cfg.embed_chans, 
                                                  depth=cfg.temporal_layers, num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                                  drop_rate=0., attn_drop_rate=0., drop_path_rate=0.)
        
        # Head
        self.shrink = nn.Linear(cfg.embed_chans, cfg.out_chans)
    ...
    def forward(self, pos_2d, rotation = None):
        ...
        #(B, T, J, C, N)   
        f = self.spatial_model(pos_2d) 
        #(B, T, J, C, N)  
        f = self.fuse_model(f, pos_2d, rotation) 
        #(B, T, J, C, N)  
        f = self.temporal_model(f)       
        
        f = rearrange(f, 'b t j c n -> (b t j n) c')
        out = self.shrink(f)
        out = rearrange(out, '(b t j n) d -> b t j d n', t=T, j=J, n=N)
        out = out + pos_2d[:,:,:,:2,:]
        return out
